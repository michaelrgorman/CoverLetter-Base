...
$else$
\usepackage{geometry}
\geometry{a4paper, top=5cm, bottom=3cm, left=2.5cm, right=2.5cm, headsep=2cm}
$endif$
$if(lang)$
\ifnum 0\ifxetex 1\fi\ifluatex 1\fi=0 % if pdftex
...
\begin{tabular}{@{}ll@{}}
      \faicon{home}       & $fromaddress$\\
      \faicon{envelope}   & $fromemail$\\
      \faicon{phone}      & $fromphone$\\
      \faicon{github}      & $fromgithub$\\
\end{tabular}}}
...
$include-before$
$endfor$
\begin{letter}{$toname$\\$toaddress$\\$tocompany$}
\setstretch{1}
\vspace*{2cm}
\opening{$opening$}
\setstretch{$spacing$}
